...
const RegTeam = inject('store')(observer(({ id, store }) => {
    ...
    const selectFriends = () => {
            bridge.send("VKWebAppGetFriends", { multi: true }).then(data => {
                const users = data.users.slice(0, 15)
                if(data.users.length > 15){
                    setStatus({ type: 'warn', title: 'Достигнуто ограничение', text: 'Команда может состоять не более чем из 15 человек'})
                }
